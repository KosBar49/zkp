...
from zkps.zkp_log_discrete import DiscreteLogInteractive
from statistics import median

def test_performance(max_bits=20, step=2, simulations=5, zkp_class=DiscreteLogInteractive):
    x_ = []
    y_r = []
    y_v = []
    ...
    for bits in range(3, max_bits + 1, step):
        ...
        for i in range(simulations):
            ...
            y = pow(g, x, p)
            zkp_instance = zkp_class(g, y, p, x)
            t = zkp_instance.commitment()
            ver = zkp_class(g, y, p)
            c = ver.challenge()
            s_r = time.time()
            s = zkp_instance.response(c)
            e_r = time.time()
