...
from zkps.zkp_log_discrete import DiscreteLog
from statistics import median

def test_performance(max_bits=20, step=2, simulations=5, zkp_class=DiscreteLog):
    x_ = []
    y_r = []
    y_v = []
    ...
    for bits in range(3, max_bits + 1, step):
        ...
        for i in range(simulations):
            ...
            y = pow(g, x, p)
            client_a = zkp_class(g, y, p, x)
            client_b = zkp_class(g, y, p)
