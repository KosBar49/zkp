...
from zkps.zkp_pederesen_commitments_messages import PederesenCommitmentsEqualMessagesEcc as zkp_class
...
def test_performance(max_bits=20, step=2, simulations=5, zkp_class=zkp_class):
    ...
    for bits in range(3, max_bits + 1, step):
        ...
        for i in range(simulations):
            ...
            P = curve.point_add(curve.scalar_mult(x, g1), curve.scalar_mult(y, h1))
            Q = curve.point_add(curve.scalar_mult(x, g2), curve.scalar_mult(z, h2))
            
            client_a = zkp_class(x, y, z)
            client_b = zkp_class()
